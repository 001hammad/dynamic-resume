...
interface ResumeDataTypes  {
 ...
 const generateResume = () => {
     const skillsHTML = resumeForm.skill.map(skill => `<ul><li><p> ${skill}</p></li></ul>`).join('');
         ...
         <div class='main-output'>
             <div class='left-sec'>
                 ...
                 <div class='left-inner-content'>
                     <p><u><h4 class="Professional-Summary">Professional Summary</h4></u><br>${resumeForm.summary}</p>
                     <p><u><h4 class="Career-Objective">Career Objective</h4></u><br>${resumeForm.objective}</p>
                 </div>
             </div>
 
             <div class='right-sec'>
                 <div class='right-inner-content'>
                     <h1 class="title">${resumeForm.title}</h1>
                     <h3 class='ts-headings'>Personal Information</h3>
                     <p><b>Full Name:</b> ${resumeForm.name}</p>
                     <p><b>Email Id:</b> ${resumeForm.email}</p>
                     <p><b>Address:</b> ${resumeForm.address}</p>
                     <p><b>Contact Number:</b> ${resumeForm.phone}</p><hr>
                     <h3 class='ts-headings'>Education</h3>
                     <p><b>Institution:</b> ${resumeForm.institution}</p>
                     <p><b>Degree:</b> ${resumeForm.degree}</p>
                     <p><b>Passing Year:</b> ${resumeForm.year}</p><hr>
 
                     <h3 class='ts-headings'>Skills</h3>
                     ${skillsHTML}
 
                     <h3 class='ts-headings'><b>Work Experience</b></h3>
                     <p><b>Job Title:</b> ${resumeForm.jobtitle}</p>
                     <p><b>Company:</b> ${resumeForm.company}</p>
                     <p><b>Year:</b> ${resumeForm.year}</p>
                     <h3 class="ts-headings"><b>Description</b></h3>
                     <p> ${resumeForm.description}</p>
                 </div>
             </div>
         </div>
     `;
